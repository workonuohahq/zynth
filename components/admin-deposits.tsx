      ...
      <section className="admin-card">
        ...
        <div className="admin-table withdrawal-table">
          {rows.map(r=><div className="admin-row" key={r.id}>
            <div className="admin-person"><span className="avatar"><Clock3 size={14}/></span><span><b>{money(r.amount)}</b><small>{r.full_name||r.email||r.user_id} · {date(r.created_at)}</small>{r.payment_reference&&<small>Payment ref: {r.payment_reference}</small>}{r.strategy_name&&<small>Strategy: {r.strategy_name}</small>}{r.investment_intent&&<small>Investment funding · activates on confirmation</small>}{r.user_note&&<small>Note: {r.user_note}</small>}</span></div>
