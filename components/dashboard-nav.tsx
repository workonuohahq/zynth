...
import { Activity, CircleHelp, Flag, Home, LogOut, Settings, ShieldCheck, UserRound, Vault } from "lucide-react";
  ...
  return <>
    <aside className="dashboard-sidebar">
      ...
      <nav className="dashboard-links">
        <Link href="/dashboard/account" className={"nav-item " + (pathname === "/dashboard/account" ? "active" : "")}><Settings size={17}/><span>Account</span></Link>
    ...
    <nav className="mobile-nav">{[...links, { href: "/dashboard/account", label: "Account", icon: Settings }].map(({href,label,icon:Icon}) => <Link key={href} href={href} aria-label={label} className={pathname === href ? "active" : ""}><Icon size={18}/><span>{label}</span></Link>)}</nav>
  </>;
}