...
export async function POST(request: Request) {
  try {
    ...
    if(error){const status=/INSUFFICIENT|INVALID_AMOUNT|USER_NOT_FOUND|WITHDRAWAL_TOO_SMALL|AUTHORIZATION|PENDING_DEPOSIT_EXISTS|PENDING_WITHDRAWAL_EXISTS/.test(error.message)?400:503;return NextResponse.json({error:"Unable to submit withdrawal.",code:error.message},{status});}
