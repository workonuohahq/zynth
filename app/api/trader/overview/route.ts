...
export async function GET(){
 ...
 if(ids.length){
   const {data,error}=await s.from("zynth_daily_reports")
     .select("id,strategy_id,report_date,opening_balance,closing_balance,external_deposit,external_withdrawal,trading_pnl,return_pct,status,rejection_reason,evidence_url,note,positions_flat,submitted_at,confirmed_at")
