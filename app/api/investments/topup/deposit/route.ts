...
export async function POST(req:Request){
 try{
  ...
  const{data,error}=await s.rpc("create_topup_deposit_request",{p_user_id:user.id,p_investment_id:investmentId,p_amount:amount,p_method:"manual"});
  if(error)return NextResponse.json({error:error.message,code:error.message},{status:400});
  return NextResponse.json({ok:true,request:data});
 }catch{return NextResponse.json({error:"Unable to start top-up funding."},{status:500})}
}